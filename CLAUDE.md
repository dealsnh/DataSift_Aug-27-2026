...
**California's Assessor's Office cannot legally be searched by owner name** (confirmed live via the Assessor's own site) — a real CA statutory restriction, structurally different from Knox County TN's open Tax API name-search that `probate-property-finder` is built around. Do not assume that pattern transfers to any CA county. The fallback, the OC Clerk-Recorder's grantor/grantee deed index (`cr.occlerkrecorder.gov/RecorderWorksInternet`, records back to 1982, also needs Scrapfly's residential proxy — direct connection times out from a non-residential network, a real firewall block not a protocol issue), is a tabbed jQuery UI where **the Name-search fields are only interactable after clicking the "Name" tab first** (`a.tabItem[href='#tabs-nohdr-2']`), even though the tab's markup is already present in the page's initial HTML (presence in the DOM is not the same as interactable). **The restriction is stronger than a search-input limitation — confirmed 2026-08-28 on the Assessor's own official APN lookup (`assessedvalue.ocassessor.gov`): even searching by a KNOWN APN never returns an owner name.** Its own text: "by law, we do not allow searches by owner name; in addition, the name of the property owner is not included in the search result." Owner name is withheld from display entirely on the free/official tools, not merely gated behind name-based search.

**`ParcelQuest Lite` (`assr.parcelquest.com`) is a real third-party aggregator covering all 58 CA counties, worth keeping as a reference** — free tier capped at 50 searches / 25 parcel maps a month, and it DOES expose an "Owner" field (paywalled behind a per-report purchase, "Available" as a teaser link rather than the name itself on the free tier). Gated behind a Cloudflare Turnstile widget on a Terms-of-Service acceptance form (`sitekey 0x4AAAAAADAqSTsZIXUE1Pyz`) before the actual search screen loads; the search screen itself is a heavy client-rendered SPA with no static form to target via a plain fetch. **Two fields on a ParcelQuest parcel detail page are useful even without the paywalled owner name:** an empty Sale History block (no Sale 1/2/3/Transfer rows) is an independent cross-check against a Recorder no-hits result — two different systems agreeing nothing has recorded since 1982 is a real signal, not a coincidence; and the CA Homeowner's Exemption flag (`Exempt Type: Y`, `Exempt Amt: $7,000`) is a solid owner-occupancy proxy, since that exemption only applies to an owner-occupied primary residence.

**One record proved end to end on the notice itself, but the docket claim below was never actually verified — corrected 2026-08-28.** Barbara T. Sunada probate, Case No. 30-2026-01591427-PR-LA-CMC, found via CAPublicNotices.com (the one genuine Orange County hit, publisher Orange County Reporter — itself a confirms-genuinely-OC signal). The full notice text (pulled straight from the CAPublicNotices results page, not the court site) gives: Petition for Probate filed by **Stacey Hiroki Sunada** (proposed personal representative, IAEA full authority requested), hearing October 14, 2026, 1:30pm, Dept. CM08, Costa Mesa Justice Center — 3390 Harbor Blvd, Costa Mesa CA 92626. Attorney for Petitioner: Karen L. Heffel (State Bar No. 263819), Stone, Doyle & Heffel, 35 N. Lake Ave. Suite 710, Pasadena CA 91101, (626) 449-1196. **This entry previously claimed "full live docket pulled via the single-case-number path with zero CAPTCHA fight," citing a 07/16/2026 filing date and "intestate" — that was wrong.** The saved output from that lookup attempt is the generic court landing page with no case data at all; the single-case-number path was written but never actually confirmed working end to end. No property address anywhere in the verified record — not a technical failure, California only files the Inventory & Appraisal (where real property gets listed) after Letters are granted, and the hearing hasn't happened yet. Recorder name search (full name, last-name-only, the administrator's name) came back with zero hits every time, consistent ~425-426KB no-results page size across all three confirming genuine no-results rather than a broken query.

**A candidate property surfaced 2026-08-28, unconfirmed: 921 Van Ness Ct, Costa Mesa CA 92626 (APN 418-083-49).** Found by a different agent (Manus, fed the Sunada facts above) navigating to ParcelQuest, with the user solving the Cloudflare Turnstile by hand to capture the parcel detail screenshot — the "Owner" field itself was paywalled/hidden, so this is NOT a confirmed match, only a strong circumstantial candidate: same city as the hearing venue, empty Sale History (matches the Recorder's independent zero-hits result), and the CA Homeowner's Exemption flag set (suggests owner-occupied). Plan to confirm: once Enformion creds land, search "Stacey Sunada" anchored to THIS specific address instead of just city/state — a hit confirms ownership and returns contact info in the same call; a miss means this was a coincidental match and the search reverts to name-only. **Next step to resume: an Enformion Person Search still needs `ENFORMION_AP_NAME` / `ENFORMION_AP_PASSWORD`** — the only credential still missing from the whole enrichment chain as of 2026-08-28 (`SMARTSKIP_EMAIL`/`SMARTSKIP_PASSWORD`, `TRACERFY_API_KEY`, `SERPER_API_KEY`, `SERPAPI_KEY`, `FIRECRAWL_API_KEY`, `ANCESTRY_EMAIL`/`ANCESTRY_PASSWORD` are all now set).

**Two adjacent tools looked like Enformion substitutes and are not, both confirmed live 2026-08-28.** SmartSkip's bulk-skip API requires `mailingAddress` as one of three mandatory fields (with first/last name) — it finds phones for a name+address you already have, it cannot discover an unknown address from a name alone, so it doesn't unblock this specific gap. (Also: SmartSkip authenticates via `POST /auth/signin {email, password}`, a login, not a static API key — `SMARTSKIP_EMAIL`/`SMARTSKIP_PASSWORD` are the real env vars, not `SMARTSKIP_API_KEY` as earlier notes here implied.) Tracerfy DOES have a name-only-capable path distinct from its batch endpoint — `POST /v1/api/trace/lookup/` (Instant Trace, `obituary_enricher._lookup_dm_address_tracerfy`) accepts address/city/zip as optional — but the existing call **hardcodes `state: "TN"`**, a real bug/gap for using it on any non-Tennessee record; needs a state parameter before it's usable outside Knox/Blount. The Serper+Firecrawl people-search pipeline (`obituary_enricher._lookup_dm_address_serper_firecrawl`) is viable now that both keys are set, but its address-extraction step calls out to an LLM via `ANTHROPIC_API_KEY` (unset) — Claude running the session can substitute for that extraction step manually instead of waiting on that key.

**Source map for the resume, six reference points, only two exercised so far (2026-08-28, unverified additions queued):** the OC Clerk-Recorder's RecorderWorks index (`cr.occlerkrecorder.gov/RecorderWorksInternet`, already in use above for the Sunada name search) is also the answer to the still-open "where does an OC foreclosure notice come from" question — CA forecloses nonjudicially through the Recorder (deeds of trust, substitutions of trustee, reconveyances/releases, liens, judgments, deeds recorded since 1982), never through a court docket, so there is no separate foreclosure-specific site to find. `CAPublicNotices.com` (already in use, the source of the Sunada hit) covers both Trustee/Foreclosure and Probate notice categories in one search, filterable by county/date/type, coverage depends on which newspapers participate. **`ocreporter.news/home.cfm?ref=legalnotices`** (Orange County Reporter, an established OC legal-notice publisher carrying probate administration, creditor and probate-sale notices) is untested but is the natural second aggregator to cross-check against CAPublicNotices.com's known-broken county filter — a single-publisher site may not inherit that bug, or may have its own. The court's own gateway, `occourts.org/online-services/case-access/probate-case-access`, is the documented front door to `probatepublic.occourts.org` (the single-case-number path already reverse-engineered above) and explicitly disclaims online results as non-certified — cite that disclaimer in anything that leaves the pipeline. Two Treasurer-Tax Collector pages are new ground for a future OC Tax Sale/Tax Delinquent notice type: `octreasurer.gov/taxauction` (tax-defaulted property auctions, parcel lists, results, GIS map) and `taxbill.octreasurer.gov` (secured/unsecured/supplemental tax-bill lookup) — the second is a validation check, not a title search, playing the same supporting role Knox County's open tax API plays in the TN pipeline, except read-only with no name-search confirmed yet. **`taxbill.octreasurer.gov` is a heavy client-rendered SPA, confirmed 2026-08-28** — a plain URL with an `?apn=` query param lands on the generic homepage, not a parcel result; it needs the same class of scripted Scrapfly `js_scenario` form-fill already built for the Recorder and court sites, not yet built.

**Testing the DataSift upload+list-creation mechanic (2026-08-28) surfaced that `.env`'s `DATASIFT_EMAIL`/`DATASIFT_PASSWORD` and the account `datasift_schema_setup.py` targets are two DIFFERENT DataSift accounts, not one org with a permissions gap.** `datasift_api_upload.py` mints its own JWT straight from `.env` via `POST /api/token/`; `/api/internal/user/` on that JWT resolves to `deals@nationalhouses.com` ("Tina Howard", plan tier `sensei`, its own wallet balance) — a genuinely separate account from whatever `datasift_schema_setup.py` targets, which doesn't even run on this machine (it hardcodes a path to a different machine's checkout, `C:\Users\Tyrus\OneDrive\Desktop\Deal Room Coaching Call\_api`, and reads a shared credential store there under the name `datasift-apikey`). The practical effect: the `.env` account had ZERO custom fields defined, so every `FIELD_MAP` lookup in `datasift_api_upload.py` silently skipped with a warning — not a bug, just an account that had never had schema created in it. **`src/datasift_schema_bootstrap.py`** (new) fixes this for any `.env`-authenticated account: idempotent, dry-run by default, creates a "First-to-Market" custom-field group (`POST /api/internal/custom-fields/group/` requires `{"label", "entity_type"}` — NOT `{"title"}`, learned by trial) and the 5 fields `datasift_api_upload.FIELD_MAP` needs outside the Knox-specific lien/tax fields: Notice Type (select, the 7 canonical types, lowercase to match `knox_ftm_pull.py`'s actual casing), County, Source URL, Personal Representative, Decedent Name (all plain text). Verified live: all 5 fields attach correctly after running it. **Also fixed in the same pass: `datasift_api_upload.build_property()` sent `Lists` as a raw comma-string instead of an array** — same trap already fixed for `Tags`, just never triggered before since every prior caller sent exactly one list. Verified live by dual-tagging a record into both `FTM_OCCA_TEST` and `Probate` in one upload.

**Foreclosure source map, five reference points (2026-08-28), plus the real yield measured through the first one:**
- **`capublicnotices.com`'s Trustee Sales category is genuinely low-yield for Orange County — measured, not assumed.** A naive newspaper-name check (matching "ORANGE COUNTY REPORTER") is a false-negative trap: it reported ZERO OC hits across 72 statewide-unfiltered candidates (8 monthly pulls, Jan-Aug 2026) because most OC-serving legal notices run in general CA legal papers (Daily Commerce, SF Daily Journal, San Jose Post-Record, Business Journal) that carry no county name at all. The reliable signal is the notice BODY, not the newspaper: match on the sale-location phrase (trustee auctions are legally required to be held at the county courthouse/civic center of the property's county, so "Orange Civic Center" or "300 East Chapman, Orange, CA" in the auction-location sentence is a hard confirm) or an explicit "County of Orange" / "County of said Orange" phrase in the legal description — never trust a bare city-name keyword match alone, since city names like Fullerton/Irvine/Orange also appear as trustee company names, attorney addresses, or incidental text unrelated to the property. **Measured real yield with the correct check: 1 genuine OC hit in 72 candidates (~1.4%).** One fully verified record: 1020 North Harbor Boulevard, Santa Ana, CA 92703 (APN 198-011-06), Trustor Helena Pham, beneficiary Velocity Commercial Capital LLC dba New Day Commercial Capital, Trustee Beacon Default Management Inc., sale 8/20/2026 at Orange Civic Center, Deed of Trust recorded 9/26/2024 as Instrument No. 2024000250998, unpaid balance $3,434,147.76 — note the loan size and lender name mark this as a commercial/investment property, not a typical residential target, so it's real but may not fit every buy box.
- **`www.capublicnotice.com` (singular "notice") is a DIFFERENT front-end from `capublicnotices.com` (plural) — confirmed live, not a typo.** Both run on the same underlying vendor platform ("iPublish® Media Solutions," per the singular site's footer) but appear to be separate deployments: the singular site's Trustee Sale category page (`capublicnotice.com/category/legals/notice-of-trustee-sale`) showed 169 results for a SINGLE day (08/27/2026), an order of magnitude more volume than the plural site's typical 9-18 results per multi-week query. **Its county dropdown filter genuinely works, confirmed 2026-08-29** — selecting Orange correctly cascaded the Source/Newspaper dropdown down to the 20 real OC-serving papers (Anaheim Bulletin, Orange County Register, Fullerton News Tribune, etc.), unlike the plural site's broken filter. ~~**But its `firstDate`/`lastDate` query params do NOT appear to control the live search**~~ **CORRECTED 2026-09-02: the date params work fine. The search is SESSION-GATED, and a cold request returns zero rows no matter how correct the parameters are.** Load a landing page (`/`) in the same browser context first, then `/search/query` honors `firstDate`/`lastDate` exactly. Verified by running identical queries with and without the warm-up: fresh context returned 0 rows for every window including the ones that had just worked. Reading cold-session emptiness as "the filter is broken" is what produced the 4-week-window theory; there is no rolling window and no hidden archive path. See "Orange County CA, second pass" below for the full working contract.
- **The OC Clerk-Recorder's RecorderWorks (`cr.occlerkrecorder.gov/RecorderWorksInternet`) has a dedicated Document Type search tab, confirmed to exist but not yet returning results.** Beyond the Name tab already in use for the Sunada search, the same jQuery tab UI has a "Document Type" tab (`a.tabItem[href='#tabs-nohdr-4']`) with a From/To recording-date range and a scrollable checkbox list of ~300+ document type codes (filterable via `#fltdocType`) — the relevant codes are `NT TRUSTEE SALE` (checkbox value `210`, the actual foreclosure-sale notice), `APPOINT TRUSTEE`, `CTF OF TRUSTEE`, `RESIGN TRUSTEE`, and `TRUSTEES DEED`. This is the authoritative, complete, no-lottery source for OC foreclosures (every real filing, not a newspaper-participation sample) but attempts to drive it via Scrapfly `js_scenario` (both native `click` steps and direct `execute` JS setting `checked=true` + calling the page's own `documentTypes.OnSetSelection()` handler, then invoking `search.OnSearch('.searchByDocType', '&SetCriteria=true')` directly) left the results container (`#MainContent_ResultsContainer1_CtrlWidget`) empty every time — unclear whether a required field is still missing, the submit needs different timing, or the real search fires through an AJAX call this scripting approach doesn't correctly trigger. Also discovered on the same tab set and not yet tried: a **Parcel Number** search tab (`SearchByParcelNumber1`) — a direct APN search that could independently confirm or refute a ParcelQuest candidate address without needing ParcelQuest's own paywalled owner field.
- **`ocreporter.news/home.cfm?ref=legalnotices`** (Orange County Reporter) — still untested directly, but the user separately found its owner/operator claims subscription access to "an Orange County foreclosure-listing and public-record database" distinct from its plain legal-notice search. Worth checking what that separate database actually offers before assuming its notice search is the only thing it provides.
- **`octreasurer.gov/taxauction`** is for tax-DEFAULTED property auctions specifically (unpaid property taxes leading to a county-run sale), not lender/trustee foreclosures — a different notice type (Tax Sale) with a different legal mechanism, don't conflate the two when scoping a pull.
- **Full-scan result, 2026-08-29: 135 candidates scanned across 11 non-contiguous months (Feb 2025 - Aug 2026), exactly 2 genuine Orange County trustee-sale hits, only 1 usable.** The second hit (City of Orange, APN 093-241-40, Denicola Family Trust, confirmed via explicit "County of Orange" legal description) is disqualified on two independent grounds: the property is listed as **"VACANT LAND"** with no street address at all, and its sale date (11/12/2025) had already passed by the time it was found — a stale, completed foreclosure, not an active lead. Both disqualifiers matter for any future automated version of this search: filter out records with no usable street address, and drop any `auction_date` already in the past (the same rule `consolidate_foreclosures.py` already applies to the TN pipeline).

**Sunada probate record: fully resolved and uploaded, 2026-08-29.** Enformion Person Search (anchored to city="Costa Mesa", state="CA" — full street-line anchoring wasn't needed since "Sunada" is a rare surname, unlike the common-name case documented under the legacy Enformion heir section) returned exactly one match each for Barbara and Stacey Sunada, both real hits, not misses. **Barbara's own address history independently confirmed 921 Van Ness Ct, Costa Mesa CA 92626** as her most recent address (reported through 7/1/2026, single family, county Orange) — a second, independent system agreeing with the ParcelQuest candidate found earlier, closing the loop with real confidence rather than circumstantial matching. **One Enformion Person Search call returns phones AND emails together, not just address history** — `persons[0]["phoneNumbers"]` and `["emailAddresses"]` at the top level of the parsed response carry the full contact list (Stacey's record: 5 phones, 4 emails), separate from the per-address `phoneNumbers` sub-arrays; `enformion_heir.person_search()` doesn't currently surface these as named return fields, so a caller has to know to look at the raw dict. **`isCurrentPropertyOwner` came back "Not Available" on both records** — not populated at this account's data tier; don't build logic that depends on it, use address-recency + cross-system corroboration instead (which is what actually worked here). Trestle-scoring Stacey's 5 numbers found one clear standout: (626) 429-3252, Mobile, AT&T, score 100 -> Dial First, versus all 4 others scoring exactly 30 (landline/VOIP) -> Dial Fourth — a concrete illustration of why a raw phone list needs scoring before dialing, not just collecting.

**Two DataSift upload gotchas caught finishing this record:** first, the record was uploaded via a hand-built CSV rather than run through `datasift_formatter._build_tags()`, so it initially missed the canonical `FTM`, `deceased`, `high_confidence`, and `has_dm_address` tags that the real automated pipeline would have added automatically — a reminder that a one-off/manual upload needs to replicate that function's exact logic, not just tag by hand from memory, or it silently drifts from the account's real tagging convention. Second, **fixing it live proved `tags` accumulate exactly like `lists` do, they do NOT overwrite on a re-POST/upsert** — adding the canonical `orange` tag left the earlier non-canonical `Orange County` tag sitting right next to it rather than replacing it. No "remove tag" endpoint has been found yet; a stale/renamed tag has to be either accepted as clutter or actively hunted down and removed some other way.

## Orange County CA, second pass: a working probate pull, and foreclosure ruled out (build 1.0.45, 2026-09-02)

`src/occa_ftm_pull.py` (notices) + `src/occa_address_resolve.py` (property addresses) turn the August research thread into a running pipeline. Source is **`www.capublicnotice.com`, the SINGULAR domain**, chosen because every result row carries a `.location` field, so Orange County is established by the SOURCE rather than by the body-text heuristics the plural site forces (measured there at ~1.4% yield, 1 usable hit in 72 candidates). Each record is confirmed OC three independent ways: the row's `.location`, an OC-serving newspaper, and the `30-YYYY-NNNNNNNN-PR-xx-CMC` OC Superior Court case pattern.

**THE SEARCH IS SESSION-GATED, AND THAT IS WHAT THE AUGUST NOTES GOT WRONG.** A cold GET to `/search/query` returns ZERO rows no matter how correct the parameters are; load a landing page in the same browser context first and the same query works. The earlier "the date params don't control the live search, it must be locked to a rolling 4-week window with a hidden archive" conclusion was cold-session emptiness misread as absence of data. There is no rolling window. Three more behaviours, each of which also reads as "no data": **`size` works and `page` does not** (`page=1` returns `page=0`'s rows verbatim, `page=2` jumps correctly — ask for one oversized page instead, `size=9000` returns all ~7,100 OC notices for 12 months in a single request); **the category taxonomy is untrustworthy** (category 8 "Notice to Creditors" returns name-change orders for OC), so the pull takes the whole county feed for a window and classifies client-side on notice text using the skill's verbatim INCLUDE/EXCLUDE phrases; and **notices republish weekly under a fresh advert id**, so dedup is on the T.S./case number (122 probate publications collapse to 45 estates, 43 foreclosure rows to 16 properties).

**OC FORECLOSURE IS NOT AVAILABLE FROM ANY NEWSPAPER AGGREGATOR. Measured, not assumed: 16 distinct properties in 12 months, ALL 16 disqualified** — 15 auction dates already passed (newest publication 2026-04-02) and 1 vacant land with no street address. OC-serving papers stopped syndicating trustee sales after **2025-12-25**; zero in all of 2026 under categories 14 and 15, while OC published 497 notices in August alone, so the county is active and the foreclosures specifically are gone. Three of the apparent 2026 hits were HOA **delinquent-assessment** liens, a different legal mechanism, not deed-of-trust foreclosures. The authoritative source remains the Clerk-Recorder Document Type search (`NT TRUSTEE SALE`, code 210) — CA forecloses nonjudicially through the Recorder, never a court docket — and that is still unbuilt.

**Probate parsing, three fixes found by running real notices:** OC writes `CASE# 30-2026-01591281-PR-LA-CMC` and sometimes `CASE#01591887`, so a regex expecting `CASE NO` misses both and every republication then looks like a distinct estate; the PR must be anchored on "requests **that** NAME be appointed as personal representative", because the "filed by NAME in the Superior Court" form breaks when the source collapses a line break without a space ("filed by Rene Mentein the Superior Court"); and a loose `Petitioner:` fallback drags in the attorney block and mailing address, so candidates are name-shape-validated.
...
**Two bugs in `drive_uploader.py` meant this had almost certainly NEVER worked, including for the TN pipeline.** `SCOPES` was `drive.file`, which only reaches files the app itself created — a folder a human shares with the service account through the Drive UI is invisible under it, and the API reports that as a bare **404 "File not found" on the folder id**, indistinguishable from a wrong id. And `files().create()` lacked `supportsAllDrives=True`, required because the target is a **Shared Drive**; the tell is `files().get()` resolving the folder while `create` 404s on the same id. Also on a Shared Drive the service account has **`canDelete=False` but `canTrash=True`**, so cleanup must set `trashed: true` rather than call `delete`.

**Retry is not optional on this workstation.** The first upload of the run died on `ConnectionResetError` / WinError 10054 — the same Windows TLS flakiness already documented against `chat.googleapis.com`. `upload_file` now retries 3x with backoff; without it roughly one file per run is silently lost.

**Rules from Ty (2026-09-02):** **CSV only, never the JSON siblings** (`upload_outputs(csv_only=True)` filters centrally so call sites can keep passing both). **Upload only the FINISHED CSV, the one that has property addresses** — so `occa_address_resolve.py` uploads and `occa_ftm_pull.py` does not (its `--drive` is opt-in), while pipelines whose pull output already carries an address upload normally. **The dated batch identity lives in the Drive FILENAME (`occa_probate_resolved_20260901.csv`), never as a DataSift list.**

**In DataSift, use the existing canonical type lists and never create a per-pull list** — Probate goes to `Probate`, Pre-Foreclosure to `Pre-Foreclosure`, and a record that qualifies for several gets all of them. Dated lists fragment the CRM and, worse, a record parked in one never enters the niche sequential funnel, which keys off the type lists. Batch traceability comes from the `pulled_<date>` and `YYYY-MM` tags `datasift_formatter._build_tags()` already stamps. **The list endpoint returns only 10 of 53 lists and silently ignores `page`/`page_size` — use `?limit=500`.** Reading that short response as the full inventory is how a `Probate` list that had existed all along got reported as missing; check the `count` field before ever concluding a list is absent. Confirmed present: `Probate`, `Foreclosure`, `Pre-Foreclosure`, `Tax Delinquent`, `Eviction`, `Divorce`, plus `Inherited`, `Estate and Heirs`, `Pre-Probate/Deceased`, `Obituary`, `Vacant`, `Free & Clear`. Absent: `Tax Sale`, `Code Violation` (there is a `Code Enforcement`).

**THE UPLOAD ENDPOINT MOVED, AND THE FAILURE LOOKS LIKE A BILLING PROBLEM (2026-09-02).** `POST /property/`, the bare Open-API route the uploader had always used, now returns **403 "You do not have permission to perform this action" on the minted user JWT — on GET as well as POST** — for an account that is active, `is_trial: false`, on the `business` plan, holding the `upload` permission, with a positive balance. Nothing about the 403 says "wrong endpoint". `POST /api/internal/property/` takes the identical payload and returns 201. `datasift_api_upload` now posts there.

**The internal route does NOT upsert.** Where the old one silently absorbed a repeat, this one returns **400 `{"non_field_errors":["Property address already exists!"],"property":["<uuid>"]}`** — with the existing uuid in the body. `_existing_uuid()` reads it back out and the run treats it as a find, not a failure, so re-running a file stays idempotent and still attaches custom fields and notes to the record already there. The summary counts `already-existed` separately, and the "created nothing from a non-empty file is a failure" guard now counts created + existing.

**Verification gotcha that nearly produced a false alarm: the property LIST serializer carries no `tags` or `lists` keys at all**, only `tag_count` / `list_count`. Scanning all 2,200 properties for a batch tag therefore returns zero even when every record is correctly tagged. Tags, lists and custom fields are only visible on the per-record detail route (`/api/internal/property/<uuid>/` and `.../custom-field/`, the latter paginated under `results`). `?search=` is also ignored on that endpoint — it returns unfiltered rows. To verify a batch, re-POST a sample, take the uuid out of the 400, and read each record back.

**Live result:** 43 records, `created=42 already-existed=1 custom-fields-set=43 failed=0`, property count 2157 -> 2200. A 6-record random audit read back 6/6 on the `Probate` list, 6/6 with the full canonical tag set, 6/6 with all 5 custom fields. DataSift's own enrichment then added its own lists on top (`Owner Occupied`, `Free & Clear`, `High Equity`, `Absentee Owners`), which is the multi-list behaviour we want and arrives without being asked.

**EVERY PIPELINE NOW REPORTS ITSELF: ONE NOTIFICATION PER COMPLETED BATCH (2026-09-02).** The whole Orange County run — pull, address resolve, 43-record upload, Priority 1 tagging — went out **completely silent**, and Ty had to ask where the notification was. The webhook was fine the entire time; nothing was wired to it. Only `main.py` and `dropbox_watcher.py` ever notified, and even `main.py` needs `--notify-slack`. **A new pipeline inherits nothing.**

The structural reason it could not simply be called: **`send_slack_notification()` takes `list[NoticeData]`**, which no CSV/dict pipeline ever builds. `slack_notifier.send_batch_summary(title, stats, detail=, warnings=, links=)` is the generic counterpart, and `datasift_api_upload` also notifies from inside `upload_rows()` so **every** DataSift upload reports regardless of caller. Wired into the OC pull and the address resolve (which attaches the Drive link). Suppress with `SIFTSTACK_NO_NOTIFY=1`. The rule Ty set is **one message per completed batch, never per record**.

**`datasift_add_tag.py` deliberately does NOT notify (2026-09-03).** It was wired the same way at first, then removed on review: the upload step had already told Chat these 43 records exist, and a second message saying "...also got tagged Priority 1" is noise, not new information to anyone reading the channel. **The refined rule: one notification per batch of NEW information reaching the team, not one per script invocation.** A follow-on step that only re-touches records another step already reported (a retag, a re-tier, a status flip on the same batch) verifies itself in the terminal/exit code instead of Chat. Applying this test up front avoids over-notifying the channel into background noise as more follow-on steps get added.

**A REAL BUG THIS EXPOSED, AND THE REASON NOTIFICATIONS WERE ALREADY BEING LOST:** `_send_webhook`'s retry loop ran twice but `except Exception: return False` bailed on the first transport error, so **only HTTP 429 was ever retried and a dropped connection was never retried at all**. On this workstation that matters constantly — the first TLS connection to a Google endpoint gets reset often enough that it cost a Drive upload per run and was silently eating notifications the same way. Now 3 attempts with backoff on the transport too; the very next test run logged `attempt 1/3 failed (ConnectionError), retrying` and then succeeded, which is how often it happens.

**`.gitignore` had a real hole:** a bare `.env` does not match `.env.bak`, and a backup taken before editing credentials is exactly as sensitive as the original. Now covers `.env.*` (keeping `!.env.example`), `*.bak-<date>`, and the `<project>-<12 hex>.json` shape Google service account keys download as.

## Notifications: Slack, Discord, and now Google Chat (build 1.0.44, 2026-08-29)

`src/slack_notifier.py` kept its name and every existing Slack/Discord code path untouched, but now also auto-detects a **Google Chat** space webhook (`GOOGLE_CHAT_WEBHOOK_URL`, URL contains `chat.googleapis.com`) off the same one-URL-many-providers dispatch pattern the module already used for Discord's `/slack` compatibility endpoint. A new `_default_webhook_url()` helper is the single place that resolves which webhook to use — **Google Chat takes priority over `SLACK_WEBHOOK_URL` when both are set**, so switching an operator's channel is a `.env` change, not a code change, and every call site (`main.py`'s `--notify-slack`, the Apify Actor path, `ftm_runner.py`'s scheduled-run summary) picked it up automatically with no per-call-site rewrite. Google Chat (like Discord) doesn't understand Slack's Block Kit `blocks`, so it gets the same plain-text fallback Discord already had. Verified live end-to-end: a real `send_slack_notification()` call built from actual Sunada record data (address, decedent, DM, cost breakdown) rendered correctly in the target Google Chat space.

**Found and fixed a real, previously-silent production bug in the same pass.** The "ZERO notices scraped" critical alert in `main.py` (the one specifically meant to catch a repeat of the 19-day dead-scrape incident from July 2026) was calling `_send_webhook(config.SLACK_WEBHOOK_URL, "alert text")` — backwards from the function's actual `(text, webhook_url)` signature. Every time that alert should have fired, it instead tried to POST to the alert text as if it were a URL, failed, and was silently swallowed by the surrounding `except Exception`. Fixed now; the alert was never reaching anyone.

**One local-only gotcha, not a code bug:** the first Python `requests` call to `chat.googleapis.com` on this Windows machine failed with a TLS `ConnectionResetError` while the identical request via `curl` succeeded immediately (HTTP 200), and a second Python attempt also succeeded. Same class of transient Windows/TLS flakiness as the `curl --ssl-no-revoke` issue hit earlier in this project's history — if a webhook call fails exactly once and then works on retry, don't go looking for a code bug.

**Multi-space routing (2026-09-03): notifications are no longer one webhook for the whole project.** Ty asked for Deep Prospecting to report to its own Google Chat space, separate from FTM/Orange County's. `GOOGLE_CHAT_WEBHOOK_URL_DEEP_PROSPECTING` in `.env` holds it; `src/obituary_dp_input.py` (Stage A) and `src/obituary_dp_run.py` (Stages B-E) pass it explicitly via `send_batch_summary(..., webhook_url=...)`. **The routing rule for any future third space: resolve that pipeline's own env var and SKIP if it's unset — never let the call fall through to `_default_webhook_url()`**, which still resolves to the FTM/OC space and would silently cross-post a different pipeline's notifications into the wrong room. Both DP scripts print `notification skipped: GOOGLE_CHAT_WEBHOOK_URL_DEEP_PROSPECTING not set` rather than defaulting. **A pasted `chat.google.com/app/chat/<id>` link is not a webhook** — it's just the space's viewing URL; the postable one (`chat.googleapis.com/v1/spaces/<id>/messages?key=...&token=...`) has to be generated from inside that space (space name -> Apps & integrations -> Webhooks -> Add webhook).

**The blank-key trap, found and partially fixed 2026-08-29:** `os.getenv(key, default)` only falls back to `default` when the key is ABSENT from `.env` — a key present with a blank value (`KEY=` with nothing after it) returns `""` and silently defeats the default. Fatal wherever that string gets parsed as a number: a blank `DROPBOX_POLL_INTERVAL` took down **every** `python src/main.py` command with `int("")` raising at import time. `src/config.py` now has a module-level `env(key, default)` helper (`(os.getenv(key) or "").strip() or default`) and routes every numeric `.env` parse through it. **THE WORKING COPY SILENTLY REVERTED AND THE FIXED BUG CAME BACK (2026-09-03).** That fix shipped in commit `ec875dd` on 2026-08-29, yet five days later `python src/main.py` died again on the exact original error, with `src/config.py` on disk carrying the pre-fix `int(os.getenv("DROPBOX_POLL_INTERVAL", "900"))` at line 55 while HEAD carried the corrected `int(env(...))` at line 66. The repo lives under `e:\OneDrive\...`, so OneDrive restoring an older file version is the likely cause. **When a bug that CLAUDE.md records as fixed reappears, run `git status` and `git diff` before rewriting any code** — the answer may be that the working tree is behind its own commit, and re-typing the fix by hand (which is what happened here) hides the sync problem instead of surfacing it. **`src/sms_agent/config.py` still has the old, unfixed pattern** — a blank `SMS_AGENT_PHASE` crashes the whole SMS agent CLI the same way, and a blank `SMS_AGENT_DRY_RUN` is worse: the guard evaluates to `False`, which **inverts the dry-run default from on to off** — a safety-relevant bug, not just a crash, since it means an unset-but-present `SMS_AGENT_DRY_RUN=` line would let the agent send live messages when the intent was always to default to dry-run. Needs the same `_env()` fix ported over before that agent is trusted with a blank config file.
...
**The v5 stack:** SmartSkip ($0.15/hit, relatives + phones) -> Tracerfy ($0.02, gap-fill only for relatives SmartSkip named but left phoneless, ~7%) -> **obituary/web research (mandatory, free)** for date of death + true relationships -> TrestleIQ ($0.015/number) for dial tiers. One record end to end is **~$0.24**. Skill: `Skills for REI/improved/deep-prospecting-v5.skill`; distributable runner `scripts/smartskip_trace.py`; API contract `references/smartskip-api.md` + the `reference_smartskip_api` memory.

**The runner this business actually uses is two `src/` scripts, not the skill's distributable copy (clarified 2026-09-03).** `scripts/smartskip_trace.py` above is `skills/deep-prospecting-v5/scripts/smartskip_trace.py` — self-contained stdlib/`urllib` only, meant to run on a community member's own machine with their own keys, with zero wiring to `src/slack_notifier.py` or anything else internal. The internal pipeline is `src/obituary_dp_input.py` (Stage A: builds the SmartSkip input CSV from the ranked obituary-opportunity list, splits out entity owners and unusable names) followed by `src/obituary_dp_run.py` (Stages B-E: rank relatives as DMs, Tracerfy gap-fill, Trestle score, report contacts-per-record). Both now post a batch-completion summary to the Deep Prospecting Google Chat space (see the Notifications section above) — the second one's message always carries a reminder that Stage C, the obituary/web research below, still has to happen before anything gets mailed.
...
*(The next two sentences describe the generic DataSift concept and the folder names `datasift_uploader` was originally written against. They are NOT what National Houses runs — see the `Hottest - NN` correction below.)* DataSift's niche sequential system uses filter presets to guide records through SMS → Call → Mail → Deep Prospecting phases. Two preset folders: "00 Niche Sequential Marketing" (12 presets, courthouse data) and "01. Bulk Sequential Marketing" (9 presets, bulk data). All 21 presets exclude Sold status (build 1.0.23). A "Sold Property Cleanup" sequence in the Transactions folder auto-fires on "Sold" tag to change status, remove from lists, clear tasks, and clear assignee.
...
**THE LIVE CADENCE IS `Hottest - NN`, AND IT IS ALREADY BUILT BY HAND (verified 2026-09-03).** Ty set it up from <https://learn.datasift.ai/niche-sequential-marketing>. Do not offer to build it, and do not describe it from `src/niche_sequential.py`. The real preset names are corroborated by production code that has pulled live cohorts (`src/sms_agent/campaign.py:51-58`): `Hottest - 02 Ready to Call`, `Hottest - 03 Call Attempt 1`, `Hottest - 04 Call Attempt 2`, `Hottest - 05 Call Attempt 3`, plus `<Prospector> - Actively Prospecting`. Every Hottest preset **excludes `new_lead`** (`engine.py:357`), and that exclusion is load-bearing: moving a record to `new_lead` fires a sequence that reassigns it away from the prospector. The article's model is **two channels running at once** — a staged call cadence and a 6-piece mail rotation both starting Day 1 (`Hottest Mail 01 Handwritten Letter`) — not the linear SMS -> Call -> Mail -> DP funnel described above.

**`src/niche_sequential.py` models a cadence that was never built.** Its twelve presets (`00. Needs Skip Traced`, `01. Ready to Text`, `02-04. Needs Called Day 1/2/3`, `05. Needs Mailed`, ...) and its 3-day cycle are fiction. This is the honest explanation for why its advancement tags — `called_day1/2/3`, `mailed`, `cycle_complete` — have **no writer anywhere in `src/`**: they belong to the invented cadence, not the real one. Real advancement runs through the Call Attempt stages. The `sequence_templates.py` vocabulary (`new_lead`, `contacted`, `offer_pending`, `dp_in_progress`) is a THIRD, unrelated naming set; the presets and the 26 sequence templates were designed independently and do not interlock. Re-base or retire the module; do not write tags nothing reads.

**`discover_presets()` CANNOT SEE THE REAL PRESETS, AND ITS ZERO LOOKS EXACTLY LIKE AN EMPTY ACCOUNT.** `src/datasift_uploader.py:2685` does not enumerate folders — it loops a hardcoded `known_folders` list holding only variants of `"00 Niche Sequential Marketing"` / `"01. Bulk Sequential Marketing"`. Nothing matches `Hottest`, so it logs `Found 0 preset folders`, and the generic Load-menu fallback returns only UI chrome (`Load, Save, Save New, Clear, Build your own filter presets...`) which reads convincingly as a fresh account. **A zero from this function is not evidence of absence.** Compounding it: `.env` authenticates to `deals@nationalhouses.com` ("Tina Howard", uuid `da2bdeae-1012-4dd6-8d61-7d0818d99dec`, confirmed live via `Api().call("/api/internal/user/")`), the sparse account that also had zero custom fields until `datasift_schema_bootstrap.py` ran — while the SMS agent pins `SMS_AGENT_SIFT_ACCOUNT = "datasift-apikey"` (ty+2). Auditing the wrong account and then trusting a name-matching scraper is a repeatable way to report working infrastructure as missing. Sequence folders that DO exist in the `.env` account: Acquisitions, default, Lead Management, Transactions (`Deep Prospecting` absent).
...
- `GOOGLE_CHAT_WEBHOOK_URL` — Google Chat space webhook for run summaries; takes priority over `SLACK_WEBHOOK_URL` when both are set (build 1.0.44, 2026-08-29 — `src/slack_notifier.py` kept its name and its Slack/Discord code paths untouched, just added Google Chat as a third auto-detected target off the same webhook-URL dispatch pattern it already used for Discord)
